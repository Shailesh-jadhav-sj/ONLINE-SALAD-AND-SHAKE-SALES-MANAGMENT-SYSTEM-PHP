...
$q = "SELECT count(*) as 'count' FROM `tbl_subscriptionmaster` where `tbl_subscriptionmaster`.`Payment_status`=1  and  SubscriptionMaster_ID=".$_GET['id'];
...
if ($result1->num_rows > 0) {
    ...
    while ($row = $result1->fetch_assoc()) {
        $q2 = "SELECT SUM(Price) as 'count2' FROM `tbl_subscriptionmaster` where `tbl_subscriptionmaster`.`Payment_status`=1 and  SubscriptionMaster_ID=".$_GET['id']." ";
    ...
    $q2 = "SELECT count(*) as 'count',Customer_id FROM `tbl_subscriptionmaster` where `tbl_subscriptionmaster`.`Payment_status`=1 and  SubscriptionMaster_ID=".$_GET['id']."  group by Customer_id order by count desc limit 1 ";
    ...
    $q3 = "SELECT start_date,Customer_id FROM `tbl_subscriptionmaster` where `tbl_subscriptionmaster`.`Payment_status`=1 and  SubscriptionMaster_ID=".$_GET['id']." group by Customer_id order by start_date limit 1 ";
    ...
    $s1 = "select * from tbl_customer_master where Customer_Id in (select Customer_id  from tbl_subscriptionmaster where tbl_subscriptionmaster.Payment_status=1 and  SubscriptionMaster_ID=".$_GET['id']." ) group by Customer_id ";
    ...
    if ($result19->num_rows > 0) {
        ...
        while ($row19 = $result19->fetch_assoc()) {
            $s20 = "select sum(Price) as 's' from tbl_subscriptionmaster where SubscriptionMaster_ID=".$_GET['id'];
            ...
            $pdf->Cell(50, 6, 'Transaction ID', 1, 0, 'C');
            $pdf->Cell(25, 6, 'price', 1, 1, 'C');
            $pdf->SetX(18);
            $pdf->SetTextColor(0, 0, 0);
        ...
        if($row['t_id']=="")
        {
            $pdf->Cell(50, 6,"Ofline payment", 1, 0, 'C');
        }
        else
        {
         $pdf->Cell(50, 6, $row['t_id'], 1, 0, 'C');
        }       
        $pdf->Cell(25, 6, $row['Price'], 1, 1, 'C'); 
